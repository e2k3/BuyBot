   ...
   if(message.content.startsWith("$buy")){
    ...
    setTimeout(() => {
     ...
     message.channel.sendEmbed(fouroulou).then(msg => {
      message.channel.awaitMessages(response => response.content === 'SBot Great.' || 'SBot Perfect.' || 'SBot Exellent.' || 'SBot Good.' && filter,{
        max: 1,
        time: 90000,
        errors: ['time']
      })
        ...
        if(collected.first().content === '$Great') {
          ...
          .setDescription(`**If you want to buy rank :**
                ...
                ** To:<@428568185277775872>**
                 **Ex: $trans <@428568185277775872> 400**
                  **To Exit Write Cancel**`)   
      .setTimestamp()
      
      msg.channel.send(`كردت اس بوت\`${Price}\` لديك 4 دقائق لتحويل
      إلى ${msg.guild.members.get(id)}**`)
      .then( msgs =>{
      const filter = response => response.author.id == "553212598368337921" && response.mentions._content.includes(`You transferred 400 to <@${client.users.get("428568185277775872").id}>`);
      msg.channel.awaitMessages(filter, { maxMatches: 1, time: 500000, errors: ['time'] })  
        ...
        if(collected.first().content === '$Perfect') {
          ...
          .setDescription(`**If you want to buy rank :**
                 ** Please transfer the amount below :**
                 \`300-Sbot\`
                ** To:<@428568185277775872>**
                 **Ex: $trans <@428568185277775872> 300**
                  **To Exit Write Cancel**`)   
      .setTimestamp()
      ...
      إلى ${msg.guild.members.get(id)}**`)
      .then( msgs =>{
      const filter = response => response.author.id == "553212598368337921" && response.mentions._content.includes(`You transferred 300 to <@${client.users.get("428568185277775872").id}>`);
      msg.channel.awaitMessages(filter, { maxMatches: 1, time: 500000, errors: ['time'] })  
          ...
          .setDescription(`**If you want to buy rank :**
                 ** Please transfer the amount below :**
                 \`200-Sbot\`
                ** To:<@428568185277775872>**
                 **Ex: $trans <@428568185277775872> 200**
                  **To Exit Write Cancel**`)   
      .setTimestamp()
      ...
      إلى ${msg.guild.members.get(id)}**`)
      .then( msgs =>{
      const filter = response => response.author.id == "553212598368337921" && response.mentions._content.includes(`You transferred 200 to <@${client.users.get("428568185277775872").id}>`);
      msg.channel.awaitMessages(filter, { maxMatches: 1, time: 500000, errors: ['time'] })  
          ...
          .setDescription(`**If you want to buy rank :**
                 ** Please transfer the amount below :**
                 \`100-Sbot\`
                ** To:<@428568185277775872>**
                 **Ex: $trans <@428568185277775872> 100**
                  **To Exit Write Cancel**`)   
      .setTimestamp()
      ...
      إلى ${msg.guild.members.get(id)}**`)
      .then( msgs =>{
      const filter = response => response.author.id == "553212598368337921" && response.mentions._content.includes(`You transferred 100 to <@${client.users.get("428568185277775872").id}>`);
      msg.channel.awaitMessages(filter, { maxMatches: 1, time: 500000, errors: ['time'] }) 
